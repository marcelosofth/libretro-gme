...
/* TODO: ajuste o caminho abaixo (e o include-dir/link do seu build) para onde
   o SDK do FluidSynth 2.6.1 estiver de fato dentro do projeto */
#include <fluidsynth.h>

/* SoundFont embutido (GeneralUser GS), gerado por embed_soundfont.py a partir
   do .sf2 original -- usado como fallback quando nao ha .sf2 externo */
#include "fluid_soundfont.inc"
...
static tsf              *g_tsf    = NULL;     /* sintese GM (SoundFont) da faixa atual, via TSF */
static tsf              *g_sf_master = NULL;  /* SoundFont carregado uma vez, compartilhado entre as faixas (TSF) */
static char              g_sf_path[700] = "";
static fluid_settings_t *g_fluid_settings = NULL;
static fluid_synth_t    *g_fluid  = NULL;     /* sintese GM (SoundFont) da faixa atual, via FluidSynth */
static int               g_gm     = 0;        /* 1 = o sintetizador ativo e via SoundFont (TSF ou FluidSynth) */
static int               g_synth_pref   = MIDI_SYNTH_AUTO;  /* escolha manual do usuario */
static int               g_active_synth = MIDI_SYNTH_MT32; /* o que realmente foi aberto */
...
extern "C" void midi_backend_close(void)
{
   ...
   if (g_fluid)
   {
      delete_fluid_synth(g_fluid);
      g_fluid = NULL;
   }
   if (g_fluid_settings)
   {
      delete_fluid_settings(g_fluid_settings);
...
/* ------------------------------------------------------------ MIDI comum (General MIDI) via FluidSynth */

/* "arquivo virtual" que o FluidSynth abre no lugar de um .sf2 de verdade quando
   nao ha nenhum externo: os callbacks abaixo leem direto do array embutido
   (fluid_sf2_data) ou, para um caminho de disco normal, do arquivo real --
   assim o mesmo loader serve os dois casos. */
#define FLUID_EMBEDDED_SF2_TAG "\x01" "embedded-generaluser-gs.sf2"
...
static void *fluid_vfile_open(const char *filename)
{
   FluidVFile *f = (FluidVFile *)malloc(sizeof(FluidVFile));
   if (!f)
      return NULL;

   if (filename && !strcmp(filename, FLUID_EMBEDDED_SF2_TAG))
   {
      f->is_memory = 1;
      f->data = fluid_sf2_data;
      f->size = (long)FLUID_SF2_SIZE;
      f->pos  = 0;
      f->fp   = NULL;
      return f;
   }
...
static int fluid_vfile_read(void *buf, fluid_long_long_t count, void *handle)
{
   FluidVFile *f = (FluidVFile *)handle;
   if (f->is_memory)
   {
      if (count < 0 || f->pos + count > f->size)
         return FLUID_FAILED;
      memcpy(buf, f->data + f->pos, (size_t)count);
      f->pos += (long)count;
      return FLUID_OK;
   }
   return fread(buf, 1, (size_t)count, f->fp) == (size_t)count ? FLUID_OK : FLUID_FAILED;
}

static int fluid_vfile_seek(void *handle, fluid_long_long_t offset, int origin)
{
   FluidVFile *f = (FluidVFile *)handle;
   if (f->is_memory)
   {
      long base = (origin == SEEK_SET) ? 0 : (origin == SEEK_CUR) ? f->pos : f->size;
   ...
   return fseek(f->fp, (long)offset, origin) == 0 ? FLUID_OK : FLUID_FAILED;
}

static fluid_long_long_t fluid_vfile_tell(void *handle)
{
   FluidVFile *f = (FluidVFile *)handle;
   return f->is_memory ? (fluid_long_long_t)f->pos : (fluid_long_long_t)ftell(f->fp);
}

static int fluid_vfile_close(void *handle)
{
   FluidVFile *f = (FluidVFile *)handle;
   if (!f->is_memory && f->fp)
      fclose(f->fp);
...
static int open_fluid(const char *sysdir)
{
   char sf[700];
   const char *path;
   ...
   sf[0] = 0;
   external = sysdir && sysdir[0] && find_sf2(sysdir, sf, sizeof(sf));
   path = external ? sf : FLUID_EMBEDDED_SF2_TAG;

   g_fluid_settings = new_fluid_settings();
   if (!g_fluid_settings)
   {
      midi_backend_close();
      return -6;
   }
   fluid_settings_setnum(g_fluid_settings, "synth.sample-rate", (double)g_rate);
   fluid_settings_setint(g_fluid_settings, "synth.midi-channels", 16);

   g_fluid = new_fluid_synth(g_fluid_settings);
   if (!g_fluid)
   {
      midi_backend_close();
      return -6;
   ...
   /* registra nosso proprio loader (dados embutidos + fallback pra arquivo real),
      testado antes do loader padrao que o FluidSynth ja adiciona sozinho */
   loader = new_fluid_defsfloader(g_fluid_settings);
   if (!loader)
   {
      midi_backend_close();
      return -6;
   ...
   fluid_sfloader_set_callbacks(loader,
                                 fluid_vfile_open, fluid_vfile_read,
                                 fluid_vfile_seek, fluid_vfile_tell, fluid_vfile_close);
   fluid_synth_add_sfloader(g_fluid, loader);

   if (fluid_synth_sfload(g_fluid, path, 1) == FLUID_FAILED)
   {
      midi_backend_close();
      return -6;
   }
...
static void fluid_deliver(const MidiEvent &e)
{
   if (e.kind == 0)
   {
      unsigned st = (unsigned)(e.a & 0xFF);
      ...
      {
         case 0x80:
            fluid_synth_noteoff(g_fluid, ch, d1);
            break;
         case 0x90:
            if (d2)
               fluid_synth_noteon(g_fluid, ch, d1, d2);
            else
               fluid_synth_noteoff(g_fluid, ch, d1);
            break;
         case 0xB0:
            fluid_synth_cc(g_fluid, ch, d1, d2);
            break;
         case 0xC0:
            fluid_synth_program_change(g_fluid, ch, d1);
            break;
         case 0xE0:
            fluid_synth_pitch_bend(g_fluid, ch, (unsigned)(d1 | (d2 << 7)));
            break;
         default:
            break;
      }
   ...
   else if (e.kind == 1)
   {
      ...
      unsigned long n = e.b;
      /* g_song->sysex guarda o F0 inicial em p[0]; o FluidSynth espera o conteudo
         a partir do byte seguinte (id do fabricante em diante) */
      if (n > 1)
         fluid_synth_sysex(g_fluid, (const char *)p + 1, (int)n - 1, NULL, NULL, NULL, 0);
   }
}
...
{
   if (g_active_synth == MIDI_SYNTH_FLUIDSYNTH)
      return g_fluid && fluid_synth_get_active_voice_count(g_fluid) > 0;
   if (g_tsf)
...
static int open_mt32(const char *sysdir)
{
...
extern "C" int midi_backend_open(const unsigned char *data, long size, long sample_rate, const char *sysdir)
{
   ...
   /* modo automatico: classifica pelo conteudo (MT-32 x GM), como antes;
      modo manual: usa exatamente o sintetizador escolhido pelo usuario */
   if (g_synth_pref == MIDI_SYNTH_AUTO)
      use = (classify(*g_song) == MIDI_MODE_GM) ? MIDI_SYNTH_TSF : MIDI_SYNTH_MT32;
   else
      use = g_synth_pref;

   switch (use)
   {
      case MIDI_SYNTH_TSF:        rc = open_gm(sysdir);   break;
      case MIDI_SYNTH_FLUIDSYNTH: rc = open_fluid(sysdir); break;
      default:                    use = MIDI_SYNTH_MT32; rc = open_mt32(sysdir); break;
   }

   g_active_synth = (rc == 0) ? use : MIDI_SYNTH_MT32;
   g_gm = (rc == 0 && use != MIDI_SYNTH_MT32) ? 1 : 0;
   return rc;
}

extern "C" void midi_backend_set_synth(int synth)
{
   if (synth < MIDI_SYNTH_AUTO || synth > MIDI_SYNTH_FLUIDSYNTH)
      synth = MIDI_SYNTH_AUTO;
   g_synth_pref = synth;
}

extern "C" int midi_backend_active_synth(void)
{
   return g_active_synth;
}
...
extern "C" int midi_backend_render(short *out, int frames)
{
   ...
   while (n < frames)
   {
      ...
      if (g_active_synth == MIDI_SYNTH_FLUIDSYNTH)
         fluid_synth_write_s16(g_fluid, chunk, out + (size_t)n * 2, 0, 2, out + (size_t)n * 2, 1, 2);
      else if (g_tsf)
