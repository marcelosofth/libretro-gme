...
static bool stopped_;   /* true = parado pelo botao Stop (diferente de apenas pausado) */
...
/* ---- navegador: escolher outro arquivo dentro do .zip atual ---- */

static char current_path_[512];
static char temp_zip_path_[512];
static char temp_dir_[512];
static bool have_temp_zip_ = false;
static int  temp_counter_  = 0;

static zip_entry **browse_entries_ = NULL;
static int         browse_count_   = 0;
static char        browse_dir_[512] = "";  /* pasta atual dentro do zip, ex: "" ou "Jogos/Fase1/" */

/* lembranca da ultima pasta aberta e do ultimo arquivo tocado no navegador,
   para reabrir a janela (SELECT) direto de onde parou, dentro do MESMO zip/arquivo atual */
static char last_browse_dir_[512]    = "";
static char last_selected_name_[300] = "";
static char up_from_name_[300]       = "";  /* nome da pasta de onde saimos ao apertar B (subir um nivel) */
...
{
   snprintf(temp_dir_, sizeof(temp_dir_), "%s", dir ? dir : "");
}
...
void player_cleanup_temp(void)
{
   if (have_temp_zip_)
   {
      remove(temp_zip_path_);
      have_temp_zip_ = false;
   }
}
...
int browser_open(void)
{
   ...
   if (!current_path_[0])
      return -1;
   /* tenta reabrir na ultima pasta usada; se nao existir mais, cai para a raiz */
   snprintf(browse_dir_, sizeof(browse_dir_), "%s", last_browse_dir_);
   if (!list_zip_entries(current_path_, browse_dir_, &browse_entries_, &browse_count_))
   {
      browse_dir_[0] = '\0';
      if (!list_zip_entries(current_path_, browse_dir_, &browse_entries_, &browse_count_))
         return -1;
...
/* posicao, na listagem atual, do ultimo arquivo tocado (0 se nao houver ou nao for encontrado) */
int browser_last_cursor(void)
{
   int i;
   if (!last_selected_name_[0])
      return 0;
   for (i = 0; i < browse_count_; i++)
      if (browse_entries_[i] && strcmp(browse_entries_[i]->name, last_selected_name_) == 0)
         return i;
   return 0;
}
...
{
   if (!browse_entries_ || index < 0 || index >= browse_count_ || !browse_entries_[index])
      return "";
   return browse_entries_[index]->name;
}
...
int browser_entry_is_zip(int index)
{
   if (!browse_entries_ || index < 0 || index >= browse_count_ || !browse_entries_[index])
      return 0;
   return browse_entries_[index]->is_zip;
}

int browser_entry_is_dir(int index)
{
   if (!browse_entries_ || index < 0 || index >= browse_count_ || !browse_entries_[index])
      return 0;
   return browse_entries_[index]->is_dir;
}
...
/* re-lista o conteudo de browse_dir_ (usado ao entrar/sair de pastas) */
static bool browser_relist(void)
{
   zip_entry **new_entries = NULL;
   int new_count = 0;
   if (!list_zip_entries(current_path_, browse_dir_, &new_entries, &new_count))
      return false;
   free_zip_entries(browse_entries_, browse_count_);
   browse_entries_ = new_entries;
   browse_count_   = new_count;
   return true;
}

/* sobe uma pasta dentro do navegador. retorna a nova contagem, ou -1 se ja estava na raiz */
int browser_up(void)
{
   size_t len;
   char *slash, *start;
   if (!browse_dir_[0])
      return -1;
   len = strlen(browse_dir_);
   if (len > 0 && browse_dir_[len - 1] == '/')
      browse_dir_[len - 1] = '\0';
   /* guarda o nome da pasta que estamos deixando, para achar ela na listagem de cima
      e reposicionar o cursor nela (em vez de sempre voltar pro topo) */
   slash = strrchr(browse_dir_, '/');
   start = slash ? slash + 1 : browse_dir_;
   snprintf(up_from_name_, sizeof(up_from_name_), "%s", start);
   if (slash)
      slash[1] = '\0';
   else
      browse_dir_[0] = '\0';
...
/* posicao, na listagem de cima, da pasta de onde acabamos de sair (0 se nao encontrar) */
int browser_up_cursor(void)
{
   int i;
   if (!up_from_name_[0])
      return 0;
   for (i = 0; i < browse_count_; i++)
      if (browse_entries_[i] && browse_entries_[i]->is_dir &&
          strcmp(browse_entries_[i]->name, up_from_name_) == 0)
         return i;
   return 0;
}
...
{
   free_zip_entries(browse_entries_, browse_count_);
   browse_entries_ = NULL;
   browse_count_   = 0;
   browse_dir_[0]  = '\0';
}

/* entra na pasta escolhida, toca a faixa (arquivo normal ja carregado),
   ou extrai e entra no zip aninhado escolhido (substitui a playlist inteira, como um novo load) */
void browser_select(int index)
{
   zip_entry *e;
   char full_name[600];
   if (!browse_entries_ || index < 0 || index >= browse_count_)
      return;
   e = browse_entries_[index];
   if (!e)
      return;

   if (e->is_dir)
   {
      size_t dlen = strlen(browse_dir_);
      size_t nlen = strlen(e->name);
      if (dlen + nlen + 2 <= sizeof(browse_dir_))
      {
         memcpy(browse_dir_ + dlen, e->name, nlen);
         browse_dir_[dlen + nlen]     = '/';
         browse_dir_[dlen + nlen + 1] = '\0';
      }
      browser_relist();
      return;
   }

   snprintf(full_name, sizeof(full_name), "%s%s", browse_dir_, e->name);
   ...
   if (!e->is_zip)
   {
      ...
      if (!plist)
         return;
      for (i = 0; i < plist->num_tracks; i++)
      {
         gme_track_data *t = plist->tracks[i];
         gme_file_data *f  = t ? plist->files[t->file_id] : NULL;
         if (f && f->name && strcmp(f->name, full_name) == 0)
         {
            snprintf(last_browse_dir_, sizeof(last_browse_dir_), "%s", browse_dir_);
            snprintf(last_selected_name_, sizeof(last_selected_name_), "%s", e->name);
            start_track(i);
            return;
         }
      }
      log_cb(RETRO_LOG_ERROR, "[GME] '%s' nao foi encontrado na playlist atual.\n", full_name);
      return;
   }
   else
   {
      char *data = NULL;
      int len = 0;
      if (!extract_zip_entry(current_path_, full_name, &data, &len))
      {
         log_cb(RETRO_LOG_ERROR, "[GME] Falha ao extrair '%s' do zip.\n", full_name);
         return;
      }
      {
         char candidate[512];
         char dirbuf[512];
         const char *dir = temp_dir_[0] ? temp_dir_ : dirname_of(current_path_, dirbuf, sizeof(dirbuf));
         FILE *fp;
         temp_counter_++;
         snprintf(candidate, sizeof(candidate), "%s/gme_zb_%08x.zip", dir,
                  ((unsigned)temp_counter_) ^ (unsigned)time(NULL));
         fp = fopen(candidate, "wb");
         if (!fp)
         {
            log_cb(RETRO_LOG_ERROR, "[GME] Nao foi possivel criar arquivo temporario em '%s'.\n", dir);
            free(data);
            return;
         }
         ...
         free(data);
         {
            bool had_old = have_temp_zip_;
            char old_temp[512];
            if (had_old)
               snprintf(old_temp,sizeof(old_temp),"%s",temp_zip_path_);
            close_file();
            snprintf(temp_zip_path_, sizeof(temp_zip_path_), "%s", candidate);
            have_temp_zip_ = true;
            if (!open_file(temp_zip_path_, sample_rate_))
               log_cb(RETRO_LOG_ERROR, "[GME] Falha ao carregar o zip selecionado.\n");
            if (had_old)
               remove(old_temp);
         }
      }
...
bool open_file(const char *path, long sample_rate)
{
   ...
   snprintf(current_path_, sizeof(current_path_), "%s", path ? path : "");
   current_track = 0;
   prev_fileid   = -1;
   last_browse_dir_[0]    = '\0';
   last_selected_name_[0] = '\0';
...
void start_track(int tracknr)
{
   ...
   if(track)
   {
      ...
      if (file->is_tracker)
      {
         ...
         if (xmp_backend_open((const unsigned char*)file->data, file->length, sample_rate_, file->name) == 0)
         {
            use_xmp_    = true;
            is_playing_ = true;
         }
      ...
      if (use_midi_)
      {
         if (midi_backend_ended())
         {
            if (loop_enabled_)
               ...
               start_track(0);   /* fim da playlist: volta pro primeiro arquivo (loopa se so houver 1) */
         }
         else
...
/* ---- troca manual do sintetizador MIDI (botao X), preservando a posicao ---- */

static int next_manual_synth(int current)
{
   switch (current)
   {
      case MIDI_SYNTH_MT32:       return MIDI_SYNTH_TSF;
      case MIDI_SYNTH_TSF:        return MIDI_SYNTH_FLUIDSYNTH;
      case MIDI_SYNTH_FLUIDSYNTH: return MIDI_SYNTH_MT32;
      default:                    return MIDI_SYNTH_MT32;   /* vindo de MIDI_SYNTH_AUTO */
   }
}
...
void cycle_midi_synth(void)
{
   ...
   if (!use_midi_ || seek_active_)
      return;

   midi_backend_set_synth(next_manual_synth(midi_backend_active_synth()));

   target = raw_elapsed_ms();
   ...
   internal_restart_ = 1;
   start_track(current_track);        /* reabre com o novo sintetizador (g_synth_pref ja atualizado) */
   internal_restart_ = 0;
...
void next_track(void)
{
   if(current_track< (plist->num_tracks-1))
      ...
      start_track(0);   /* ultima faixa: R da a volta pro comeco da playlist */
}
...
void prev_track(void)
{
   if(current_track > 0)
      ...
      start_track(plist->num_tracks - 1);   /* primeira faixa: L da a volta pro fim da playlist */
}
...
void play_pause(void)
{
   ...
   if (stopped_)
   {
      /* estava parado (Stop): apertar Play/Start volta a tocar do inicio da faixa atual */
      internal_restart_ = 1;
      start_track(current_track);
      internal_restart_ = 0;
      return;
...
static void detect_chips(void)
{
   ...
   if (file->is_midi)
   {
      int synth = midi_backend_active_synth();
      if (synth == MIDI_SYNTH_FLUIDSYNTH)
         strcpy(chip_text, "GM (FluidSynth)");
      else if (synth == MIDI_SYNTH_TSF)
         strcpy(chip_text, "GM (TSF)");
      else
   ...
   if (file->is_tracker)
   {
      strcpy(chip_text, xmp_backend_uses_amiga_mix() ? "AMIGA: MIX. PAULA" : "TRACKER");
      return;
   }
