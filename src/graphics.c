...
/* mistura a cor "fg" com o pixel de fundo "bg" ja existente na tela.
   alpha: 0 = totalmente transparente (mantem o fundo), 255 = totalmente opaco (cor pura) */
unsigned short blend_color(unsigned short bg, unsigned short fg, unsigned char alpha)
{
#ifdef ABGR1555
   int bg_r =  bg        & 0x1F;
   int bg_g = (bg >> 4)  & 0x3E;
   ...
   return (unsigned short)((b << 20) | ((g & ~1) << 4) | r);
#else
   int bg_r = (bg >> 21) & 0x1F;
   int bg_g = (bg >> 5)  & 0x3F;
   int bg_b =  bg        & 0x1F;
   ...
   int g = (fg_g * alpha + bg_g * (255 - alpha)) / 255;
   int b = (fg_b * alpha + bg_b * (255 - alpha)) / 255;
   return (unsigned short)((r << 21) | (g << 5) | b);
#endif
}

/* igual ao draw_shape, mas misturando com o que ja esta desenhado (transparencia) */
void draw_shape_alpha(surface *surf, unsigned short color, unsigned char alpha, int pos_x, int pos_y, int w, int h)
{
   int y, x;
   unsigned short bg;
   for(y=pos_y;y<(pos_y+h);y++)
   {
      for(x=pos_x;x<(pos_x+w);x++)
      {
         bg = get_pixel(surf,x,y);
         set_pixel(surf,x,y,blend_color(bg,color,alpha));
      }
   }
}
